import React from 'react'

interface IconProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string
}

const LocationTickOutline = ({ size = 24, ...props }: IconProps): React.ReactElement => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    aria-hidden={true}
    focusable="false"
    {...props}
  >
<path d="M12 22.76C10.52 22.76 9.02999 22.2 7.86999 21.09C4.91999 18.25 1.65999 13.72 2.88999 8.33C3.99999 3.44 8.26999 1.25 12 1.25C12 1.25 12 1.25 12.01 1.25C15.74 1.25 20.01 3.44 21.12 8.34C22.34 13.73 19.08 18.25 16.13 21.09C14.97 22.2 13.48 22.76 12 22.76ZM12 2.75C9.08999 2.75 5.34999 4.3 4.35999 8.66C3.27999 13.37 6.23999 17.43 8.91999 20C10.65 21.67 13.36 21.67 15.09 20C17.76 17.43 20.72 13.37 19.66 8.66C18.66 4.3 14.91 2.75 12 2.75Z" fill="currentColor"/>
<path d="M10.75 13.7499C10.56 13.7499 10.37 13.6799 10.22 13.5299L8.72 12.0299C8.43 11.7399 8.43 11.2599 8.72 10.9699C9.01 10.6799 9.49 10.6799 9.78 10.9699L10.75 11.9399L14.22 8.46994C14.51 8.17994 14.99 8.17994 15.28 8.46994C15.57 8.75994 15.57 9.23994 15.28 9.52994L11.28 13.5299C11.13 13.6799 10.94 13.7499 10.75 13.7499Z" fill="currentColor"/>
  </svg>
)

LocationTickOutline.displayName = 'LocationTickOutline'

export { LocationTickOutline }
export default LocationTickOutline
