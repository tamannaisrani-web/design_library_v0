import React from 'react'

interface IconProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string
}

const TrendDownOutline = ({ size = 24, ...props }: IconProps): React.ReactElement => (
  <svg
    width={size}
    height={size}
    viewBox="0 0 24 24"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    aria-hidden={true}
    focusable="false"
    {...props}
  >
<path d="M16.5 15.2499C16.31 15.2499 16.12 15.1799 15.97 15.0299L12.42 11.4799L11.33 13.1199C11.21 13.3099 11 13.4299 10.78 13.4499C10.55 13.4699 10.33 13.3899 10.18 13.2299L6.97995 10.0299C6.68995 9.73994 6.68995 9.25994 6.97995 8.96994C7.26995 8.67994 7.74995 8.67994 8.03995 8.96994L10.59 11.5199L11.68 9.87994C11.8 9.69994 12 9.57994 12.23 9.54994C12.46 9.52994 12.68 9.60994 12.83 9.76994L17.03 13.9699C17.32 14.2599 17.32 14.7399 17.03 15.0299C16.88 15.1799 16.69 15.2499 16.5 15.2499Z" fill="currentColor"/>
<path d="M16.5 15.25H14.5C14.09 15.25 13.75 14.91 13.75 14.5C13.75 14.09 14.09 13.75 14.5 13.75H15.75V12.5C15.75 12.09 16.09 11.75 16.5 11.75C16.91 11.75 17.25 12.09 17.25 12.5V14.5C17.25 14.91 16.91 15.25 16.5 15.25Z" fill="currentColor"/>
<path d="M15 22.75H9C3.57 22.75 1.25 20.43 1.25 15V9C1.25 3.57 3.57 1.25 9 1.25H15C20.43 1.25 22.75 3.57 22.75 9V15C22.75 20.43 20.43 22.75 15 22.75ZM9 2.75C4.39 2.75 2.75 4.39 2.75 9V15C2.75 19.61 4.39 21.25 9 21.25H15C19.61 21.25 21.25 19.61 21.25 15V9C21.25 4.39 19.61 2.75 15 2.75H9Z" fill="currentColor"/>
  </svg>
)

TrendDownOutline.displayName = 'TrendDownOutline'

export { TrendDownOutline }
export default TrendDownOutline
